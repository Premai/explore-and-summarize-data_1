...
uplot ("fixed.acidity", 1, "fixed.acidity distribution for wine") +
...
table(Wd$residual.sugar)

table(Rd$residual.sugar)

uplot ("residual.sugar", 1, 
...
uplot ("free.sulfur.dioxide", 0.05, 
...
uplot ("total.sulfur.dioxide", 0.025, 
...
```{r density.log10}
uplot ("density", 0.0002, 
       "density of wine by log10") +
  scale_x_log10(breaks = seq(min(wine$density), 1.0051, 0.002))  
...
uplot ("sulphates", 0.01,"sulphates in wine by log10") +
...
uplot ("alcohol", 0.05,"Alcohol content in wine by log10") +
...
quplot(wine, "alcohol", "color", alcohol_mean, "Alcohol", 
...
quplot(wine, "residual.sugar", "color", ressugar_mean, "residual.sugar", 
       "Residual sugar impact on Wine Quality") +
  ylim(0, 22)
...
```{r bivar_dens_visual}
quplot(wine, "density", "color", density_mean, "Density", 
...
bioth <- function(dataset, y, gtitle) {
  ggplot(dataset,aes_string(x = "quality", y = y, fill="color")) + 
    geom_boxplot() +
    facet_grid(~color) +
    ggtitle(gtitle)
  }
...
p <- plot(wine, "chlorides", "sulphates", "color",
...
```{r w_density_alcohol}
rwcorr(cWd, "density","alcohol", "density vs. alcohol correlation for white") +
...
table(QWSub$total.sulfur.dioxide)
...
rwcorrs(cWd, "total.sulfur.dioxide","alcohol", 
...
quplot(wine, "density", "color", density_mean, "Density", "density relationship 
       with wine Quality")  +
  coord_cartesian(xlim=c(0, 9.0),
                  ylim=c(min(wine$density), 1.0025))
...
quplot(wine, "alcohol", "color", alcohol_mean, "Alcohol", 
  ...
  coord_cartesian(xlim=c(0,10), ylim=c(min(wine$alcohol),14)) 
